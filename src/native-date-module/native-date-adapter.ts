...
export class NativeDateAdapter extends DateAdapter<Date> {
  ...
  getNumDaysInMonth(date: Date): number {
    return this.getDate(this._createDateWithOverflow(
        this.getYear(date), this.getMonth(date) + 1, 0, this.getHours(date), this.getMinutes(date)));
  }
  ...
  addCalendarMonths(date: Date, months: number): Date {
    let newDate = this._createDateWithOverflow(
        this.getYear(date), this.getMonth(date) + months, this.getDate(date), this.getHours(date), this.getMinutes(date));
  ...
  private _createDateWithOverflow(year: number, month: number, date: number, hours?:number, minutes?:number) {
    let result = new Date(year, month, date, hours || 0, minutes ||0, 0, 0);
  ...
  createDate( year: number, month: number, date: number, hours?:number, minutes?:number): Date {
    ...
    let result = this._createDateWithOverflow(year, month, date, hours, minutes);
